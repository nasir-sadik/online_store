...
def showOutlet():
    ...
    return render_template('index.html', outlet=outlet, items=items)
...
def showItem(outlet_id, outlet_name):
    ...
    countitem = len(items)
    flash('Please click on OUTLETS list panel header to show all items.', "info")
    return render_template('index.html', outlet=AllOutlets, items=items, filter=outlet_name, itemCount=countitem)
...
def createNewOutlet():
    ...
    if request.method == 'POST':
        outletExist=session.query(Outlet).filter_by(name=request.form['outlet_name']).all()
...
def createNewItem():
    ...
    if request.method == 'POST':
        ...
        if itemExistance:
            flash('Sorry, an item with the name (%s) exists under the outlet you selected ' %
...
def editItem(outlet_id, item_id):
    ...
    if items.user_id != login_session['user_id']:
        flash('Sorry, you are not allowed to edit', "error")
        return redirect(url_for('showSpecificItem', outlet_name=items.outlet.name, item_name=items.name))
    if request.method == 'POST':
        ...
        if request.form['item_price']:
            ...
            return redirect(url_for('showItem',
                                    outlet_id=outlet_id, outlet_name=items.outlet.name))
    else:
...
def deleteItem(outlet_id, item_id):
    ...
    if item.user_id != login_session['user_id']:
        ...
        return redirect(url_for('showSpecificItem',
                                outlet_name=item.outlet.name, item_name=item.name))
...
# Returns all the items for a outlet in json
@app.route('/outlet/<int:outlet_id>/json')
def outletItemJSON(outlet_id):
    outlet = session.query(Outlet).filter_by(id=outlet_id).one()
    items = session.query(Item).filter_by(
        outlet_id=outlet_id).all()
...
# Return an item in json
@app.route('/outlet/<int:outlet_id>/item/<int:item_id>/json')
def itemJSON(outlet_id, item_id):
    item = session.query(Item).filter_by(id=item_id).one()
